...
exports.scrape = async function puppeteerOnlinerScraper({ writeToJson }) {
    ...
    try {
        ...
        for (let index = 0; index < scrapedData.length; index++) {
            ...
            scrapedDataDB.push(scrapedData[index]);
        }

        console.log(scrapedDataDB.length);
        if (writeToJson) {
            fs.writeFile(
                './json/scrapedData.json',
                JSON.stringify(scrapedDataDB, null, 2), // optional params to format it nicely
    ...
    for (let i = 0; i <= options.amountOfScrapedAds; i += DEFAULT_STEP) {
        ...
        $('table.ba-tbl-list__table tr').each(function (i, element) {
            ...
            if (title.length > 0 && time.length > 0) {
                ...
                let signAlreadyExists = scrapedDataDB.find((item) => {
